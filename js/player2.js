(function($) {
	...
	$.fn.videoPlayer = function(options) {
		...
		var settings = {  
			...
			videoClass : 'content'  // Video Class
		}
		...
		return this.each(function() {	
			...
			$(this)[0].addEventListener('loadedmetadata', function() {
				...
				var timeUpdate = function($ignore) {
					...
					//Duraçao total do video
					hour = Math.floor($duration / 3600);
					min = Math.floor($duration / 60);
					seg = Math.floor((($duration / 60) % 1) * 60);

					//Tempo atual		
					currentHour = Math.floor(curTime / 3600);
					currentMin = Math.floor(curTime/ 60);
					currentSeg = Math.floor(((curTime / 60) % 1) * 60);
					...
					// Update times
					$that.find('.ctime').html(convertTimer(currentHour, currentMin, currentSeg)); 
					$that.find('.ttime').html(convertTimer(hour, min, seg));
				...
				$spc.onwaiting = function(){
					var load = document.getElementById("loading");
					load.style = "display: block;";
				};
				$spc.onplaying = function(){
					var load = document.getElementById("loading");
